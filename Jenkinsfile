pipeline {
    ...
    stages {
        ...
        stage('Build Frontends (Node in Docker)') {
            steps {
...
docker version >/dev/null 2>&1 || {
  echo "ERROR: Docker is not available on this Jenkins agent"
  exit 2
}
...
build_gantt_frontend() {
  if [[ ! -f "$WS/gantt/frontend/package.json" ]]; then
    echo "SKIP gantt/frontend (no package.json)"
    return 0
  fi
...
NODE
      ...
      DST="node_modules/lightningcss/lightningcss.linux-${ARCH}-${LIBC}.node"
...
docker compose up -d --remove-orphans --scale jenkins=0
'''
                }
            }
        }
