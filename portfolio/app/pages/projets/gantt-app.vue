    ...
    <div class="max-w-5xl mx-auto space-y-10">
      <nav class="text-xs breadcrumb text-base-content/60">
        <ul>
          ...
          <li>Application de diagramme de Gantt</li>
        </ul>
      </nav>
      ...
      <header class="space-y-4">
        ...
        <h1 class="text-3xl md:text-4xl font-semibold">
          Application de diagramme de Gantt
        </h1>
        <p class="max-w-2xl text-sm md:text-base text-base-content/80">
          Une application web dédiée à la création et au suivi de diagrammes de Gantt&nbsp;:
          planification des tâches, gestion des dépendances, visualisation des jalons
          et export des plannings pour les présentations et le pilotage de projet.
        </p>
      </header>
      ...
      >
        <div class="space-y-4">
          ...
          <p class="text-sm text-base-content/80">
            L’objectif est de proposer un outil simple pour construire et ajuster un planning projet&nbsp;:
            définir des tâches, leur durée, leurs dépendances et les jalons clés, avec une visualisation
            claire sur une frise temporelle de type Gantt.
          </p>
          <p class="text-sm text-base-content/80">
            L’application est développée comme une web app autonome (front uniquement, stockage local ou mock API),
            puis intégrée dans ce portfolio via une iframe. Cela permet de la présenter comme un module technique
            isolé tout en la contextualisant dans un cas d’usage de gestion de projet.
          </p>
        </div>
        ...
        <aside class="card bg-base-200/80 border border-base-300/70 shadow-lg text-sm">
          <div class="card-body gap-3">
            ...
            <div class="space-y-1">
              ...
              <p>Conception UX, modélisation des données projet et développement front-end.</p>
            </div>
      ...
      <section class="space-y-4">
        ...
        <p class="text-sm text-base-content/80">
          L’application de diagramme de Gantt sera chargée ci-dessous comme web app autonome,
        ...
        <div class="rounded-xl border border-base-300/80 overflow-hidden bg-base-200/70">
          <div class="bg-base-300/60 px-4 py-2 text-xs flex items-center justify-between">
            <span class="text-base-content/70">
              Application de diagramme de Gantt · Web app embarquée
            </span>
          ...
          <div class="aspect-[16/9] w-full">
            <iframe
              v-if="demoUrl"
              :src="demoUrl"
              title="Application de diagramme de Gantt"
...
const techStack = ['TypeScript', 'React', 'Vite', 'Tailwind CSS', 'DaisyUI'] as const

const features: Feature[] = [
  {
    title: 'Création et édition de tâches',
    description:
      'Ajout de tâches avec titre, date de début, date de fin, responsable et catégorie (développement, recette, déploiement, communication, etc.).',
  },
  {
    title: 'Gestion des dépendances',
    description:
      'Lien entre les tâches pour visualiser les dépendances (ex. une tâche ne peut démarrer qu’après la fin d’une autre).',
  },
  {
    title: 'Vue chronologique type Gantt',
    description:
      'Affichage des tâches sur une frise temporelle avec zoom sur les jours, semaines ou mois pour s’adapter au niveau de détail souhaité.',
  },
  {
    title: 'Application front-end autonome',
    description:
      'Application web sans backend, pensée pour être facilement déployable et intégrable via iframe dans différents contextes (portfolio, démonstration, POC).',
  },
]
...
const demoUrl = computed(() => {
  const url = (config.public as Record<string, unknown>)?.ganttUrl
  return typeof url === 'string' && url.trim().length > 0 ? url.trim() : ''
})

const repoUrl = computed(() => {
  const url = (config.public as Record<string, unknown>)?.ganttRepoUrl
  return typeof url === 'string' && url.trim().length > 0 ? url.trim() : ''
})
</script>
