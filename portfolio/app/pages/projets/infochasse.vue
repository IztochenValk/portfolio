...
useSeoMeta({
  title: () => t("infochasse.seoTitle"),
  description: () => t("infochasse.seoDescription"),
})

const techStack = ["Spring Boot", "WebSocket (STOMP)", "Nuxt 3", "Leaflet", "PostGIS", "Docker", "CI/CD"] as const
const features = computed(() => tm<Feature[]>("infochasse.features"))
const demoUrl = "https://infochasse.florianchague.dev"
</script>
    ...
    <div class="max-w-5xl mx-auto space-y-10">
      <nav class="text-xs breadcrumb text-base-content/60">
        <ul>
          ...
          <li>{{ t('infochasse.pageTitle') }}</li>
        </ul>
      </nav>

      <header class="space-y-4">
        <p class="text-xs font-semibold tracking-[0.3em] uppercase text-primary">
          {{ t('infochasse.eyebrow') }}
        </p>
        <h1 class="text-3xl md:text-4xl font-semibold">{{ t('infochasse.pageTitle') }}</h1>
      ...
      <section class="grid gap-6 md:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)] items-start">
        <div class="space-y-4">
          ...
          <p class="text-sm text-base-content/80">{{ t('infochasse.objP1') }}</p>
          <p class="text-sm text-base-content/80">{{ t('infochasse.objP2') }}</p>
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          ...
          >
            <div class="card-body py-3 px-4 gap-2">
              ...
              <p class="text-xs md:text-sm text-base-content/80">{{ feature.description }}</p>
            </div>
          </div>
        </div>
      </section>
